...
BBS::BBS() {
    ...
    srand(time(nullptr));//seed the random function with the time
    p = p_numbers[rand() % p_numbers.size()];//choose random number for p and q
    q = p_numbers[rand() % p_numbers.size()];
    //check if p and q %4 ==3 if not change their value
    while (p % 4 != 3) {
        p = p_numbers[rand() % p_numbers.size()];
    }
    while (q % 4 != 3) {
        q = p_numbers[rand() % p_numbers.size()];
    }
...
//    BBS bbs;
    for (int i = 0; i < 256; ++i) {
        intial_state_vector[i] = i;//initialize S[] with 0 1 2 3..
        temp_vector[i] = bbs.bbs_key[i % 128];//initialize t[] with the key from bbs repeated
    }
    int j = 0;
    for (int i = 0; i < 256; ++i) {
        j = (j + intial_state_vector[i] + temp_vector[i]) % 256;//change the value of j each time
        permuted_state_vector[i] = intial_state_vector[j];//initialize the permuted state vector() by swapping S[i] with S[j]
        permuted_state_vector[j] = intial_state_vector[i];;
    }
}
